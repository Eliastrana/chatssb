...
export function singlePromptSchemaToPxApiQuery(responses, url: string): string {
    for (const key in responses) {

        if (responses[key].itemSelections) {
            let selection = '';
            for (const item of responses[key].itemSelections) {
                selection += item + ',';
            }
        ...
        if (responses[key].selectionExpressions) {
            for (const expression of responses[key].selectionExpressions) {
                console.log(key, 'Selection Expression:', expression);
                url += '&valueCodes[' + key + ']=[' + expression + ']';
            }
        }

        if (!responses[key].itemSelections && !responses[key].selectionExpressions) {
...
export function multithreadedPromptSchemaToPxApiQuery(
    ...
    Object.entries(responses).forEach(([responseKey, responseValue]) => {
