...
export const PxWebBubbleChart: React.FC<BubbleChartProps> = ({
    ...
    // ---- 3) Build a function to retrieve `value` from data.value ----
    ...
    // ---- 4) Pick a single time dimension value to show, e.g. the last one ----
    const selectedTimeKey = timeCategoryKeys[timeCategoryKeys.length - 1];
    const selectedTimeLabel = timeCategoryLabels[selectedTimeKey] || selectedTimeKey;

    // ---- 5) Generate combos (cartesian product) of selected non-time categories ----
    ...
    // ---- 6) Render D3 chart in useEffect ----
    useEffect(() => {
        ...
        // We’ll use a "pack" layout for bubble chart
        ...
        // Prepare root data that matches the HierarchyData interface
        ...
        const root = d3
            ...
            .sum((d) => d.value ?? 0) // The numeric value for each leaf
            .sort((a, b) => (b.value ?? 0) - (a.value ?? 0));

        const packLayout = d3.pack<HierarchyData>().size([layoutRadius * 2, layoutRadius * 2]).padding(3);
        ...
        // Leaf nodes are in packed.leaves()
        const leaves = packed.leaves(); // typed as HierarchyCircularNode<HierarchyData>[]
        ...
        const circles = svg
            ...
            .enter()
            .append("circle")
            .attr("cx", (d) => d.x - layoutRadius) // shift by layoutRadius so center is (0,0)
            .attr("cy", (d) => d.y - layoutRadius)
            .attr("r", 0) // start with zero radius for transition
            .attr("fill", (d, i) => color(String(i)))
            .on("mouseover", function (event, d) {
                tooltip.style("display", "block");

                const dataNode = d.data; // HierarchyData
                // Build label from dimension combos
                if (!dataNode.combo) return;
                const comboLabels = Object.entries(dataNode.combo)
                    .map(([dimName, catKey]) => {
                        const label = data.dimension[dimName].category.label[catKey] ?? catKey;
        ...
        // Optional: Add text labels if circles are big enough
        svg
            ...
            .style("opacity", (d) => (d.r > 20 ? 1 : 0)); // only show if circle is large enough

        // (Optional) Label the center with the time dimension
        svg
            .append("text")
            .attr("text-anchor", "middle")
            .style("font-size", "14px")
            .style("fill", "#274247")
            .text(`Time: ${selectedTimeLabel}`)
    ...
    // ---- 7) Render the dimension filters and the SVG container ----
    return (
