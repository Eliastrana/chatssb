...
You can be presented with only folders, only tables, or a mix of both, where it is your task to select the most relevant ones.
...
For each selected entry, you must return:
- \`type\`: either "Table" or "FolderInformation"
- \`id\`: the entry's unique identifier
- \`label\`: the label describing the folder or table

Only include entries that are clearly relevant. Do not add placeholders or guesses.

Begin your reasoning by fully understanding the user's request and its context from previous messages. Then, select and output the most relevant entries in structured form.
`;